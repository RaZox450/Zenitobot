...
class ControlsPredictorDot(nn.Module):
    def __init__(self, in_features, features=32, layers=1, actions=None):
        super().__init__()
        if actions is None:
            self.actions = torch.from_numpy(ZenitobotAction.make_lookup_table()).float()
        else:
...
class Zenitobot(nn.Module):  # Wraps earl + an output and takes only a single input
...
def get_critic():
    return Zenitobot(EARLPerceiver(256, 4, 8, 1, query_features=36, key_value_features=25 + 30),
                 Linear(256, 1))


def get_actor():
    split = (90,)
    return DiscretePolicy(Zenitobot(EARLPerceiver(256, 4, 8, 1, query_features=36, key_value_features=25 + 30),
...
if __name__ == '__main__':
    Zenitobot = torch.jit.load("../src/Zenitobot-model-base.pt")
    earl = EARLPerceiver(128, 1, 4, 1, query_features=32, key_value_features=24)
    splits = (90,)  # (3,) * 2 + (2,) * 3
    d = DiscretePolicy(Zenitobot(earl,
                             ControlsPredictorDot(128)), splits)
    # d = DiscretePolicy(Zenitobot(EARLPerceiver(128, 2, 4, 1, query_features=32, key_value_features=24),
